...
;;
;; Adds a new user to the credential store. If no password is given it is
...
(when (find-user users username)
  (fail (string-append "user already exists: " username
                       " (use bin/passwd.scm to change the password)")))

(save-users! users-path
             (upsert-user users username (hash-password password)
                          (exact (round (current-second)))))
(display (string-append "added user: " username "\n"))
